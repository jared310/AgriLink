...
def register():
    ...
    name, email, phone, password = data.get('name', '').strip(), data.get('email', '').strip().lower(), data.get('phone', '').strip(), data.get('password', '')
    if not name or not phone or not password: return jsonify({'message': 'Required fields missing.'}), 400
...
def login():
    data = request.json or {}
    email, phone, password = data.get('email', '').strip().lower(), data.get('phone', '').strip(), data.get('password', '')
    conn = get_db_connection()
    user = conn.execute('SELECT * FROM users WHERE email = ? OR phone = ?', (email if email else None, phone)).fetchone()
    conn.close()
    if not user or not check_password_hash(user['password'], password): return jsonify({'message': 'Invalid credentials.'}), 401
